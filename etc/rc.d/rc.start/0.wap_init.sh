...
if [ "$cut_ver" != "1" ] ; then
	...
	while true; do sleep 60; echo !stop > /proc/wap_proc/tty; echo !path "/var/console.log" > /proc/wap_proc/tty; sed 's/WAP(Dopra Linux) #//g' /var/init_debug_bak.txt > /var/init_debug.txt; break; done &	
fi
