...
HW_PDT_COLLECT_PRODUCTLIST=$workpath/productlist
HW_WAP_COLLECT_WAPLLIST=$workpath/waplist
...
echo  "Begin to collect"
    cat $HW_PDT_COLLECT_PRODUCTLIST $HW_WAP_COLLECT_WAPLLIST | while read line
    do
        echo $line | grep -o "[^ ]\+\( \+[^ ]\+\)*"
